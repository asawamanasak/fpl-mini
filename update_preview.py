  ...
  <header class="border-b border-slate-200 bg-white/95 backdrop-blur-xl sticky top-0 z-40">
    ...
    <div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 border-t border-slate-200/80">
      <nav class="flex space-x-1 sm:space-x-2 overflow-x-auto touch-scroll no-scrollbar py-1.5 sm:py-2">
        ...
        >
          <span id="tab-highlights-live-dot" class="inline-block w-2 h-2 rounded-full bg-emerald-500 animate-ping mr-1.5 align-middle hidden"></span>Weekly Highlight
        </button>
        <button 
          id="tab-btn-prizes"
    ...
    <div id="highlights-view" class="tab-content hidden space-y-4">
      ...
      <div class="glass-card rounded-2xl px-4 py-3 sm:px-5 sm:py-3.5 border border-slate-200/80 dark:border-slate-800 flex items-center justify-between gap-4">
        <div class="flex items-center gap-2.5">
          <div id="highlights-live-dot" class="w-2.5 h-2.5 rounded-full bg-emerald-500"></div>
  ...
  <script>
    ...
    class StandaloneApp {
      ...
      updateHighlightsTabBlink() {
        const tabBtn = document.getElementById('tab-btn-highlights');
        const liveDot = document.getElementById('tab-highlights-live-dot');
        const bannerDot = document.getElementById('highlights-live-dot');
        if (!tabBtn) return;
        ...
        const isActive = this.activeTab === 'highlights-view';

        // กล่อง Tab และไฟแสดงสถานะจะเปลี่ยนเป็นสีเขียวและกะพริบจนกว่าจะสรุปจบ Gameweek
        if (!isFinished) {
          if (isActive) {
            tabBtn.className = 'tab-nav-btn tab-pulse-live-active flex-shrink-0 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs sm:text-sm font-bold transition-all duration-200 bg-emerald-600 text-white border border-emerald-500 shadow-xs dark:bg-emerald-600 dark:text-white dark:border-emerald-400';
          } else {
            tabBtn.className = 'tab-nav-btn tab-pulse-live flex-shrink-0 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs sm:text-sm font-bold transition-all duration-200 text-emerald-600 dark:text-emerald-400 border border-emerald-500/80 bg-emerald-500/10 hover:bg-emerald-500/20';
          }
          if (liveDot) liveDot.classList.remove('hidden');
          if (bannerDot) {
            bannerDot.className = 'w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.8)]';
          }
        } else {
          if (isActive) {
            tabBtn.className = 'tab-nav-btn flex-shrink-0 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs sm:text-sm font-bold transition-all duration-200 bg-slate-900 text-white shadow-xs dark:bg-blue-600 dark:text-white border border-transparent';
          } else {
            tabBtn.className = 'tab-nav-btn flex-shrink-0 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all duration-200 text-slate-600 dark:text-slate-400 border border-transparent hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800';
          }
          tabBtn.classList.remove('tab-pulse-live', 'tab-pulse-live-active');
          if (liveDot) liveDot.classList.add('hidden');
          if (bannerDot) {
            bannerDot.className = 'w-2.5 h-2.5 rounded-full bg-slate-400 dark:bg-slate-600';
          }
        }
      }
